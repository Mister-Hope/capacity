...
<template>
  <div class="capacitor-container">
    <div class="bg-slate-950 border border-slate-900 p-3 rounded-2xl shadow-2xl text-slate-100 flex flex-col gap-6 select-none relative overflow-hidden">
      
      <!-- Premium Tech Ambient Lights -->
      <div class="absolute -top-40 -left-40 w-80 h-80 bg-blue-500/5 rounded-full blur-[100px] pointer-events-none"></div>
      <div class="absolute -bottom-40 -right-40 w-80 h-80 bg-amber-500/5 rounded-full blur-[100px] pointer-events-none"></div>

      <!-- Upper Visual Interactive Section -->
      <div class="relative z-10">
        
        <!-- SVG Board Graphic (Full width for enhanced projector readability) -->
        <div class="w-full bg-slate-900 shadow-2xl rounded-2xl border border-slate-800 p-2 overflow-hidden flex flex-col justify-center relative" style="max-height: 220px;">
          
          <!-- HUD 读数面板 -->
          <div class="absolute top-3 right-3 bg-slate-950/85 border border-slate-800 rounded-lg px-2.5 py-1.5 flex items-center gap-2.5 shadow-xl backdrop-blur z-20 font-sans">
          ...
          <svg viewBox="0 0 600 280" class="capacitor-svg w-full max-h-full">
            ...
            <g opacity="0.15">
              <line v-for="x in verticalGrid" :key="'v-' + x" :x1="x" y1="0" :x2="x" y2="280" stroke="#475569" stroke-width="1" stroke-dasharray="2,4" />
              <line v-for="y in horizontalGrid" :key="'h-' + y" x1="0" :y1="y" x2="600" :y2="y" stroke="#475569" stroke-width="1" stroke-dasharray="2,4" />
            </g>
            ...
            <line stroke="#475569" stroke-width="2.5" :x1="upperWireX" y1="0" :x2="upperWireX" :y2="upperPlateY" stroke-linecap="round" class="" />
            
            <!-- Wire 2: Lower plate wire going straight out to the bottom screen, connects to center of lower plate -->
            <line stroke="#475569" stroke-width="2.5" :x1="lowerWireX" y1="280" :x2="lowerWireX" :y2="lowerPlateY" stroke-linecap="round" class="" />
            ...
            <g class="">
              ...
              <rect :x="300 - 80" :y="upperPlateY - 52" width="160" height="32" rx="6" fill="#0f172a" stroke="#fda4af" stroke-width="1.5" />
              <text
                x="300"
                :y="upperPlateY - 31"
                fill="#fda4af"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 items-end">
          ...
          <div class="flex flex-col gap-0.5 w-full">
            <div class="flex justify-between items-baseline gap-1">
              ...
              <span class="text-blue-400 text-sm font-extrabold shrink-0">{{ d.toFixed(1) }} mm</span>
            </div>
            <input 
              type="range" 
              min="3.0" 
          ...
          <div class="flex flex-col gap-0.5 w-full">
            <div class="flex justify-between items-baseline gap-1">
              ...
              <span class="text-amber-400 text-sm font-extrabold shrink-0">{{ U.toFixed(1) }} V</span>
            </div>
            <input 
              type="range" 
              min="10.0"
          ...
          <div class="flex flex-col w-full">
            <div class="grid grid-cols-3 gap-1 w-full">
              <button
                @click="epsilonR = 1"
                :class="epsilonR === 1 ? 'border border-amber-500 bg-amber-500/15 text-amber-300 font-black' : 'border border-slate-800 hover:border-slate-700 hover:bg-slate-800/40 text-slate-400 font-semibold'"
                class="py-1.5 px-0.5 rounded-lg transition-all duration-200 text-center text-[10px] leading-tight"
              >真空</button>

              <button
                @click="epsilonR = 3"
                :class="epsilonR === 3 ? 'border border-blue-500 bg-blue-500/15 text-blue-300 font-black' : 'border border-slate-800 hover:border-slate-700 hover:bg-slate-800/40 text-slate-400 font-semibold'"
                class="py-1.5 px-0.5 rounded-lg transition-all duration-200 text-center text-[10px] leading-tight"
              >玻璃</button>

              <button
                @click="epsilonR = 6"
                :class="epsilonR === 6 ? 'border border-purple-500 bg-purple-500/15 text-purple-300 font-black' : 'border border-slate-800 hover:border-slate-700 hover:bg-slate-800/40 text-slate-400 font-semibold'"
                class="py-1.5 px-0.5 rounded-lg transition-all duration-200 text-center text-[10px] leading-tight"
              >陶瓷</button>
            </div>
          </div>
